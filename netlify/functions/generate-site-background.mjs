...
// Bloco de responsividade injetado em todos os prompts de geração/reparo:
...
function analyzeGeneratedSite(project, job, files) {
  ...
  // Responsividade mobile: sem viewport meta o celular renderiza como desktop
    ...
    if (outputFormat === "single-html") {
      stages = [
        {
          ...
          system: `Você é o gerador de sites premium do ForzaAI. O usuário pediu UM ÚNICO index.html self-contained: gere APENAS o index.html completo (sem markdown, sem cercas), com TODO o CSS dentro de <style> no <head> e TODO o JS dentro de <script> no fim do <body>. NÃO referencia styles.css nem script.js. ${designBrief}\nRegras: site completo e publicável; hero impactante com título + subtítulo + 2 CTAs + apoio visual; mínimo ${typeRequirements.minimums.sections + 2} seções reais (${typeRequirements.required_sections.join(", ")}) + depoimentos/FAQ quando couber + footer completo; semântico (<main>, <section>, <header>, <footer>); copy específica em português do Brasil com pelo menos ${typeRequirements.minimums.visibleText} caracteres visíveis; ${typeRequirements.minimums.cards}+ cards/itens; zero placeholders/TODO/lorem. CSS: variáveis :root, Google Fonts, Grid/Flexbox, hover/focus, microinterações, @keyframes. ${RESPONSIVE_RULES} JS: menu mobile, âncora suave, tabs/accordion, IntersectionObserver reveals, formulário com validação e feedback. ${antiTruncate}`,
        },
      ];
      ...
      stages = [
        ...
        },
        {
          ...
          system: `Você é o designer CSS do ForzaAI. Produza APENAS o styles.css completo (sem markdown, sem explicação) para o app React cujo shell HTML foi gerado. ${designBrief}\nRegras: :root com variáveis, Google Fonts (@import), Grid/Flexbox, hover/focus/active, microinterações, @keyframes, transições, mínimo ${typeRequirements.minimums.cssRules} regras, primeira dobra densa, focus-visible, prefers-reduced-motion. ${RESPONSIVE_RULES} Estilize todas as classes usadas nos componentes. ${antiTruncate}`,
        },
        {
          key: "script.js",
